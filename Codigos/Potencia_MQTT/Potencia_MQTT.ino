 ...
 * Por cada ciclo (cada 16.6 ms) se toman 19 muestras (19 * 870 us = 16.5 ms, que es menor que 1/60 s)
 * 
 * Por cada 4/60 de segundo (66.6 ms para 4 ciclos de 60Hz) se toman (19*4)-2=74 muestras
 ...
 * Se deben de tener alrededor de 14-15 calculos de potencia por segundo
 * 
 * Cada 10 segundos se realiza un promedio de todos los calculos de potencia realizados
 * 
 * Para calcular potencia se realiza lo siguiente:
 ...
 * 
 * La ganancia de voltaje se determino expermientalmente para el sensor ZMPT101B y es de alrededor de 550 veces (aproximadamente 820000/1500)
 ...
 * La ganancia de corriente, utilizando una resistencia de 1Kohm es de 2 (2000*1000)
 * 
 */
...
const int t_por_calculo_potencia = 1000000; // Cada 10 segundos se calcula un promedio de las potencias calculadas (en microsegundos)
const int t_por_ciclo = 16666; // Frecuencia de la señal electrica 60Hz (en microsegundos)
const int t_por_muestra = 870; // Tomar 19 muestras por ciclo (en microsegundos)

//Constantes para la tma de muestras
const int num_muestras = 19; // Muestras tomadas por ciclo (por cada 1/60 de segundo o 870us aproximadamente)
...
const double v_ganancia = 820000/1500; // ganancia del sensor de voltaje
const double i_ganancia = 2000/1000; // ganancia del sensor de corriente
...
void mediciones_potencia(){
  ...
  {
    potencia_prom = potencia_acc / i_potencia;
...
void enviar_datos(){
  ...
  dtostrf(potencia_prom, 1, 2, dataString);  // Esta es una función nativa de leguaje AVR que convierte un arreglo de caracteres en una variable String
  //Serial.print("Dato enciado: "); // Se imprime en monitor solo para poder visualizar que el evento sucede
  //Serial.println(dataString);
  client.publish("potencia/disp1", dataString); // Esta es la función que envía los datos por MQTT, especifica el tema y el valor
}
...
void calcular_potencia(){
  voffset = abs(maximo[0] - minimo[0])/2.0 + minimo[0];
...
void obtener_maximo_minimo(){
  ...
  for(int i = 2; i < num_muestras_tomadas - 2; i++)
  {
    ...
    if(corriente_o_voltaje == 0)
    {
      ...
      //corrientes[i] = muestras[i];
      corriente = (muestras[i] - voffset) * i_ganancia; // Se resta la componente de offset y se multiplica por la ganancia
      i2 += pow(corriente,2); // Se calcula la corriente al cuadrado
    }
    else
    {
      // Se realiza el calculo para el voltaje al cuadrado
      //voltajes[i] = muestras[i];
